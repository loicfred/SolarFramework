 ...
 * with its roster held in {@link ParticipantMember}. Matches always reference a Participant,
 * never an individual member, so the same match code serves every team size.
 */
@Entity
@Table(name = "tournament_participant")
public class Participant extends DatabaseObject.ID_RECORD_OBJ<Long, Participant> {
    ...
    private List<ParticipantMember> members = new ArrayList<>();
    ...
    public Participant(Tournament tournament, String name) {
        this.ID = Ids.next();
    ...
    public Participant(Tournament tournament, String name, int seed) {
    ...
    public List<ParticipantMember> getMembers() {
        if (members == null) members = new ArrayList<>(retrieveEntityServiceFor(ParticipantMember.class).getAllWhere("ParticipantID = ?", ID));
        return members;
    }

    public boolean removeMember(String memberName) { return getMembers().removeIf(m -> m.getName().equalsIgnoreCase(memberName)); }

    public Optional<ParticipantMember> getCaptain() { return getMembers().stream().filter(ParticipantMember::isCaptain).findFirst(); }
    ...
    private java.util.stream.Stream<Match> decidedMatches() {
        Tournament t = getTournament();
    ...
    public int getMatchesDrawn() { return (int) decidedMatches().filter(Match::isTie).count(); }
