...
public abstract class IParticipantMember extends DatabaseObject.ID_RECORD_OBJ<Long, IParticipantMember> {
    ...
    protected IParticipantMember() {}

    protected IParticipantMember(IParticipant participant, String name) {
        this.ID = Ids.next();
    ...
    public IParticipant getParticipant() {
        return participant == null ? participant = retrieveEntityServiceFor(IParticipant.class).getById(participantID).orElse(null) : participant;
    }
    public void setParticipant(IParticipant p) { this.participant = p; this.participantID = p == null ? null : p.getID(); }
