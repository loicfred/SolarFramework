 ...
 * individual games in {@link MatchGame}, and {@link #recomputeFromGames()} folds them back into the
...
public class Match extends DatabaseObject.ID_RECORD_OBJ<Long, Match> {
    ...
    public Match(Phase phase, BracketSide side, int round, int position) {
        this.ID = Ids.next();
        this.phase = phase;
    ...
    public Optional<Participant> getParticipant1() { return resolve(participantID1); }
    public Optional<Participant> getParticipant2() { return resolve(participantID2); }

    private Optional<Participant> resolve(Long id) {
        if (id == null) return Optional.empty();
        Phase p = getPhase();
        Tournament t = p == null ? null : p.getTournament();
        return t != null ? t.getParticipant(id) : retrieveEntityServiceFor(Participant.class).getById(id);
    }

    /** Winner as an entity; empty while undecided or on a draw. */
    public Optional<Participant> getWinner() { return resolve(winnerID); }
    /** Loser as an entity; empty while undecided, on a draw, or on a bye. */
    public Optional<Participant> getLoser() { return resolve(loserID); }

    public Optional<Participant> getParticipant(int slot) { return slot == 1 ? getParticipant1() : getParticipant2(); }
    ...
    public List<MatchGame> getGames() {
        ...
        games = new ArrayList<>(retrieveEntityServiceFor(MatchGame.class).getAllWhere("MatchID = ?", ID));
        games.sort(Comparator.comparingInt(MatchGame::getGameNumber));
        return games;
    }

    /** Recomputes series score, raw points and winner from the individual games. */
    public void recomputeFromGames() {
        List<MatchGame> gs = getGames();
    ...
    public String getSlotLabel(int slot) {
        Optional<Participant> p = getParticipant(slot);
    ...
    public void reportResult(int score1, int score2) {
        ...
        log.info("Match {} reported {}-{} ({})", getMatchNumber(), score1, score2, getWinner().map(Participant::getDisplayName).orElse("draw"));
        afterResult();
    }
    ...
    /** Appends one game reported as individual player contributions; each side's game score is the sum of its own members'. */
    public MatchGame reportGame(Map<ParticipantMember, Integer> playerPoints) { return reportGame(playerPoints, null); }

    public MatchGame reportGame(Map<ParticipantMember, Integer> playerPoints, String stage) {
        requirePlayable();
        requireNotClinched();
    ...
    private MatchGame finalizeGame(MatchGame g, String stage) {
        ...
        if (getGames().size() >= getBestOf() || (!decidesByPoints() && isSeriesClinched())) {
            decide();
            setState(MatchState.COMPLETE);
            log.info("Match {} complete {}-{} ({})", getMatchNumber(), getScore1(), getScore2(), getWinner().map(Participant::getDisplayName).orElse("draw"));
            afterResult();
        } else Upsert();
        return g;
    }
    ...
    /** Decides a match without play - the named entrant advances. */
    public void reportWalkover(Participant winner) {
        requirePlayable();
    ...
    void afterResult() {
        ...
        Phase p = (Phase) phase;
        List<Match> changed = PhaseEngines.of(phase.getType()).onMatchDecided(phase, this);
        Upsert();
    ...
    public void reset() {
        ...
        Tournament t = phase.getTournament();
    ...
    private void clearFeed(Phase phase, Long targetID, Integer slot, Long participantID) {
