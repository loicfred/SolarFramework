...
public class MatchGame extends DatabaseObject.ID_RECORD_OBJ<Long, MatchGame> {
    ...
    public MatchGame(Match match, int gameNumber, int score1, int score2) {
        this.ID = Ids.next();
        this.match = match;
    ...
    public Match getMatch() {
        return match == null ? match = retrieveEntityServiceFor(Match.class).getById(matchID).orElse(null) : match;
    }
    public void setMatch(Match m) { this.match = m; this.matchID = m == null ? null : m.getID(); }
    ...
    public void reportPlayerPoints(ParticipantMember member, int points) {
    ...
    private int sumFor(Participant side, Map<Long, Integer> points) {
        if (side == null) return 0;
        int total = 0;
        for (ParticipantMember member : side.getMembers()) total += points.getOrDefault(member.getID(), 0);
        return total;
    }
