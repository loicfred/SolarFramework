...
class StandingsCalculatorTest {
    ...
    void recomputeRanksARoundRobinByPointsThenTiebreakers() {
        ...
        List<Participant> field = List.of(new Participant(t, "P1", 1), new Participant(t, "P2", 2), new Participant(t, "P3", 3), new Participant(t, "P4", 4));
        ...
        for (Match m : phase.getMatches()) {
            Participant a = m.getParticipant1().orElseThrow(), b = m.getParticipant2().orElseThrow();
            boolean aWins = strength.get(a.getName()) < strength.get(b.getName());
            m.setScore(aWins ? 1 : 0, aWins ? 0 : 1);
            m.setState(MatchState.COMPLETE);
        }

        List<Standing> ranked = StandingsCalculator.recompute(phase).stream().sorted(Comparator.comparingInt(Standing::getRank)).toList();
        assertEquals(List.of("P1", "P2", "P3", "P4"), ranked.stream().map(Standing::getName).toList());
    ...
    void headToHeadBreaksAPointsTieBeforeGameDifference() {
        Tournament t = new Tournament("RR");
        Phase phase = t.addPhase("RR", PhaseType.ROUND_ROBIN);
        Participant a = new Participant(t, "A", 1), b = new Participant(t, "B", 2), c = new Participant(t, "C", 3);
        ...
        assertEquals(List.of(1, 2, 3), ranked.stream().map(Standing::getRank).toList());
        ranked.forEach(s -> assertEquals(3.0, s.getPoints()));
    }

    private void decide(Phase phase, Participant winner, Participant loser, int s1, int s2) {
        Match m = phase.getMatches().stream().filter(x -> x.hasParticipant(winner.getID()) && x.hasParticipant(loser.getID())).findFirst().orElseThrow();
