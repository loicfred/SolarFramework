...
/**
 * Runnable demo: a 2v2 double elimination tournament from registration to champion, with no application context
 * anywhere - building the renderer is the whole of the setup, which is what a plugin carrying this module does too.
 * <p>{@link Tournament_Main} is the same script under Spring Boot, where {@code TournamentConfig} builds it instead.
 */
public class Tournament_PlainMain {
    ...
    static void main(String[] args) throws Exception {
        ...
        while (t.getStatus() != TournamentStatus.COMPLETE) {
            ...
            for (Match m : playable) {
                Participant a = m.getParticipant1().orElseThrow();
                Participant b = m.getParticipant2().orElseThrow();
