...
class SingleEliminationEngineTest {
    ...
    private void playThrough(Phase phase) {
    ...
    void topSeedWinsEveryMatchAndTakesTheTitle() {
        ...
        Standing champion = engine.rank(phase).stream().filter(s -> s.getRank() == 1).findFirst().orElseThrow();
    ...
    void bothSemifinalLosersAreThirdWhenNoThirdPlaceMatchSeparatesThem() {
        ...
        engine.generate(phase, field(t, 4));
        playThrough(phase);
        assertEquals(List.of(1, 2, 3, 3), engine.rank(phase).stream().map(Standing::getRank).toList());
    }
    @Test
    void aPlayedThirdPlaceMatchSplitsTheSemifinalLosers() {
        ...
        engine.generate(phase, field(t, 4));
        playThrough(phase);
        assertEquals(List.of(1, 2, 3, 4), engine.rank(phase).stream().map(Standing::getRank).toList());
    }
    @Test
    void everyRoundOfExitsSharesAPlacingAndTheNextRankSkipsTheTie() {
        ...
        playThrough(phase);
        assertEquals(List.of(1, 2, 3, 3, 5, 5, 5, 5), engine.rank(phase).stream().map(Standing::getRank).toList());
    }
    @Test
