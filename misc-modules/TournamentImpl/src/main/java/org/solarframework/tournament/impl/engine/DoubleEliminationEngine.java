...
public class DoubleEliminationEngine extends SingleEliminationEngine {
    ...
    public void generate(Phase phase, List<Participant> entrants) {
        ...
        List<List<Match>> wb = buildSkeleton(phase, size, wbRounds, BracketSide.WINNERS);
        List<List<Match>> lb = buildLosersBracket(phase, size, wbRounds);
        wireLosersFeeds(wb, lb, size, wbRounds);

        Match grandFinal = newMatch(phase, BracketSide.GRAND_FINAL, 1, 0);
    ...
    private List<List<Match>> buildLosersBracket(Phase phase, int size, int wbRounds) {
    ...
    private List<Match> round(Phase phase, int roundNumber, int count) {
        List<Match> out = new ArrayList<>(count);
     ...
     */
    private List<Match> maybeReset(Phase phase, Match grandFinal) {
        ...
        Match reset = newMatch(phase, BracketSide.GRAND_FINAL_RESET, 1, 0);
        reset.setLabel("Grand Final (reset)");
        reset.setBestOf(grandFinal.getBestOf());
        reset.setMatchNumber(phase.getMatches().stream().mapToInt(Match::getMatchNumber).max().orElse(0) + 1);
    ...
    public boolean isComplete(Phase phase) {
        ...
        List<Match> gf = phase.getMatches(BracketSide.GRAND_FINAL);
