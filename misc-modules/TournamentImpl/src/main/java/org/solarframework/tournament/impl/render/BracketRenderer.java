...
public class BracketRenderer implements IBracketRenderer {
    ...
    @Override
    public byte[] renderPng(Phase phase, BracketTheme theme) {
    ...
    @Override
    public byte[] renderPng(Phase phase) { return renderPng(phase, BracketTheme.dark()); }

    @Override
    public byte[] renderPng(Tournament tournament, BracketTheme theme) {
        BracketTheme t = orDefault(theme);
        try {
    ...
    @Override
    public void writePng(Phase phase, BracketTheme theme, OutputStream out) throws IOException {
        BracketTheme t = orDefault(theme);
        PngPainter.write(new BracketLayout(phase, t).build(), t.getScale(), out);
    }

    @Override
    public File writePng(Phase phase, BracketTheme theme, File target) throws IOException {
        try (OutputStream out = new FileOutputStream(target)) { writePng(phase, theme, out); }
        return target;
    }

    @Override
    public String renderSvg(Phase phase, BracketTheme theme) { return SvgPainter.render(new BracketLayout(phase, orDefault(theme)).build()); }

    @Override
    public String renderSvg(Phase phase) { return renderSvg(phase, BracketTheme.dark()); }

    @Override
    public String renderSvg(Tournament tournament, BracketTheme theme) { return SvgPainter.render(stack(tournament, orDefault(theme))); }

    @Override
    public File writeSvg(Phase phase, BracketTheme theme, File target) throws IOException {
        Files.writeString(target.toPath(), renderSvg(phase, theme), StandardCharsets.UTF_8);
        return target;
    }

    @Override
    public String renderHtml(Phase phase, BracketTheme theme) {
        BracketTheme t = orDefault(theme);
        Tournament parent = phase.getTournament();
        return HtmlPainter.render(new BracketLayout(phase, t).build(), t, parent == null ? phase.getName() : parent.getName() + " - " + phase.getName());
    }

    @Override
    public String renderHtml(Phase phase) { return renderHtml(phase, BracketTheme.dark()); }

    @Override
    public String renderHtml(Tournament tournament, BracketTheme theme) {
        BracketTheme t = orDefault(theme);
        return HtmlPainter.render(stack(tournament, t), t, tournament.getName());
    }

    @Override
    public File writeHtml(Phase phase, BracketTheme theme, File target) throws IOException {
        Files.writeString(target.toPath(), renderHtml(phase, theme), StandardCharsets.UTF_8);
        return target;
    }

    @Override
    public File writeHtml(Tournament tournament, BracketTheme theme, File target) throws IOException {
    ...
    private RenderModel stack(Tournament tournament, BracketTheme theme) {
