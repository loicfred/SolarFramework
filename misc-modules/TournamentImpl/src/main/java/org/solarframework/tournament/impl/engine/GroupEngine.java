...
public class GroupEngine extends AbstractPhaseEngine {
    ...
    public void generate(Phase phase, List<Participant> entrants) {
        ...
        List<List<Participant>> split = Seeder.intoGroups(entrants, groups, snake);

        int maxRounds = 0;
        for (int g = 0; g < split.size(); g++) {
            List<Participant> members = split.get(g);
    ...
    private int resolveGroupCount(Phase phase, int entrants) {
     ...
     */
    private int scheduleGroup(Phase phase, int groupIndex, List<Participant> members) {
        List<Participant> arr = new ArrayList<>(members);
        if (arr.size() % 2 == 1) arr.add(null);
        int n = arr.size(), rounds = n - 1, half = n / 2;
        int legs = phase.isDoubleRoundRobin() ? 2 : 1;
        int position = 0;
        for (int leg = 0; leg < legs; leg++) {
            List<Participant> rot = new ArrayList<>(arr);
            for (int r = 0; r < rounds; r++) {
                for (int i = 0; i < half; i++) {
                    Participant a = rot.get(i), b = rot.get(n - 1 - i);
                    if (a == null || b == null) continue;
                    Match m = newMatch(phase, BracketSide.GROUP, leg * rounds + r + 1, position++);
    ...
    public List<Match> onMatchDecided(Phase phase, Match match) {
        StandingsCalculator.recompute(phase);
        phase.setCurrentRound(phase.getPendingMatches().stream().mapToInt(Match::getRound).min().orElse(phase.getTotalRounds()));
        return List.of();
    }
     ...
     */
    @Override
    public List<Participant> getQualifiers(Phase phase) {
        List<Standing> ranked = rank(phase);
        int perGroup = phase.getAdvancePerGroup();
        int total = phase.getEffectiveAdvanceTotal();
        List<Standing> picked = new ArrayList<>();
        for (int i = 1; i <= Math.max(1, perGroup); i++) {
            int pos = i;
            picked.addAll(ranked.stream().filter(s -> s.getRank() == pos).sorted(orderAcrossGroups()).toList());
        }
        List<Standing> qualified = picked.size() > total ? picked.subList(0, total) : picked;
        for (Standing s : ranked) { s.setQualified(qualified.contains(s)); s.setEliminated(!qualified.contains(s)); }
        return qualified.stream().map(Standing::getParticipant).flatMap(java.util.Optional::stream).toList();
    }

    /** Compares equally placed entrants from different groups - best record goes through highest. */
    private Comparator<Standing> orderAcrossGroups() {
        return Comparator.comparingDouble(Standing::getPoints).reversed()
                ...
                .thenComparing(Comparator.comparingInt(Standing::getScoreDiff).reversed())
                .thenComparingInt(Standing::getGroupIndex);
    }
}
